(function(){
  ...
  var saved = null;
  try { saved = localStorage.getItem('vyxenn-theme'); } catch(e){}
  applyTheme(saved);
  btn.addEventListener('click', function(){
    var current = root.getAttribute('data-theme') || (systemPrefersDark() ? 'dark' : 'light');
    var next = current === 'dark' ? 'light' : 'dark';
    applyTheme(next);
    try { localStorage.setItem('vyxenn-theme', next); } catch(e){}
  });
  ...
  function waLink(name, price){
    ...
    var msg = "Hi Vyxenn, I'd like to order the " + plain + " (" + fmt(price) + ").";
  ...
  function renderGrid(list, elId){
    ...
    el.innerHTML = list.map(function(p){
      return '<article class="prod-card">'
        + '<div class="prod-img"><img src="'+IMG[p.img]+'" alt="'+p.name.replace(/&amp;/g,'&')+'" loading="lazy"></div>'
        + '<h3 class="prod-name">'+p.name+'</h3>'
        + '<p class="prod-price">'+fmt(p.price)+'</p>'
        + '<a class="btn-whatsapp" href="'+waLink(p.name, p.price)+'" target="_blank" rel="noopener">Order on WhatsApp</a>'
      + '</article>';
    }).join('');
  }
  ...
  form.addEventListener('submit', function(e){
    ...
    if(!email){ msg.textContent = "Enter your email to join the list."; return; }
